// int4_ptx_mma_k32.cu
// True-k32 INT4 PTX kernel:
// - Uses native ldmatrix.x2 for A (16x32 int4 tile packed as 16x8 b16)
// - Uses native ldmatrix.x1 for each B n8-half (8x32 int4 tile packed as 8x8 b16)
// - Uses native mma.sync.aligned.m16n8k32.row.col.s32.s4.s4.s32
...
void ldmatrix_a_k32(uint32_t ra[2], const int8_t smem[][K_BYTES + PAD], int lane) {
    const int r = lane % 16;
    const uint32_t addr = __cvta_generic_to_shared(&smem[r][0]);
    asm volatile(
...
__device__ __forceinline__
void ldmatrix_b_k32(uint32_t rb[1], const int8_t smem[][K_BYTES + PAD], int lane, int n_base) {
...
__global__ void int4_ptx_mma_k32_db(
    ...
    // K loop: step by 32 int4 elements => 16 bytes per row.
...
void launch_kernel(const int8_t* A, const int8_t* BT, int32_t* C,
    ...
    int4_ptx_mma_k32_db<<<blocks, threads, 0, stream>>>(A, BT, C, cfg.M, cfg.N, cfg.K);
