// int4_ptx_3stage.cu
// 3-stage INT4 PTX kernel (k64):
// - Triple-buffered cp.async pipeline (wait_group 1)
// - Native ldmatrix.x4/.x2 fragment loads
// - Native mma.sync.aligned.m16n8k64.row.col.s32.s4.s4.s32
...
void ldmatrix_a_k64(uint32_t ra[4], const int8_t smem[][K_BYTES + PAD], int lane) {
    ...
    asm volatile(
        ...
        : "=r"(t0), "=r"(t1), "=r"(t2), "=r"(t3)
        : "r"(addr));
    // Re-order to match proven fragment mapping from int8 true-k32 / int4 k64.
    ra[0] = t0;
    ra[1] = t2;
    ra[2] = t1;
    ra[3] = t3;
...
__global__ void int4_ptx_3stage_db(
    ...
    __shared__ __align__(16) int8_t As[3][WARPS_PER_BLOCK][WMMA_M][K_BYTES + PAD];
    __shared__ __align__(16) int8_t Bs[3][WARPS_PER_BLOCK][WMMA_N][K_BYTES + PAD];

    int32_t rc0[4] = {0, 0, 0, 0};
    int32_t rc1[4] = {0, 0, 0, 0};

    // Prolog: stage 0 and 1 async fills.
    for (int i = lane_id; i < (WMMA_M * K_BYTES) / 16; i += THREADS_PER_WARP) {
        const int row = (i * 16) / K_BYTES;
        const int byte_col = (i * 16) % K_BYTES;
        cp_async16(&As[0][warp_id][row][byte_col],
                   &A_b[(tile_row + row) * (K / 2) + byte_col + 0]);
    }
    for (int i = lane_id; i < (WMMA_N * K_BYTES) / 16; i += THREADS_PER_WARP) {
        const int n = (i * 16) / K_BYTES;
        const int byte_col = (i * 16) % K_BYTES;
        cp_async16(&Bs[0][warp_id][n][byte_col],
                   &BT_b[(tile_col + n) * (K / 2) + byte_col + 0]);
    }
    ...
    for (int i = lane_id; i < (WMMA_M * K_BYTES) / 16; i += THREADS_PER_WARP) {
        const int row = (i * 16) / K_BYTES;
        const int byte_col = (i * 16) % K_BYTES;
        cp_async16(&As[1][warp_id][row][byte_col],
                   &A_b[(tile_row + row) * (K / 2) + byte_col + (WMMA_K / 2)]);
    }
    for (int i = lane_id; i < (WMMA_N * K_BYTES) / 16; i += THREADS_PER_WARP) {
        const int n = (i * 16) / K_BYTES;
        const int byte_col = (i * 16) % K_BYTES;
        cp_async16(&Bs[1][warp_id][n][byte_col],
                   &BT_b[(tile_col + n) * (K / 2) + byte_col + (WMMA_K / 2)]);
    }
    asm volatile("cp.async.commit_group;");
    ...
    ldmatrix_a_k64(ra,  As[0][warp_id], lane_id);
    ldmatrix_b_k64(rb0, Bs[0][warp_id], lane_id, 0);
    ldmatrix_b_k64(rb1, Bs[0][warp_id], lane_id, 8);

    int buf = 0;

    // Main loop: keep one group in-flight while computing.
    for (int k = 2 * WMMA_K; k < K; k += WMMA_K) {
        const int next = (buf + 1) % 3;
        const int prefetch_buf = (buf + 2) % 3;

        for (int i = lane_id; i < (WMMA_M * K_BYTES) / 16; i += THREADS_PER_WARP) {
            const int row = (i * 16) / K_BYTES;
            const int byte_col = (i * 16) % K_BYTES;
            cp_async16(&As[prefetch_buf][warp_id][row][byte_col],
                       &A_b[(tile_row + row) * (K / 2) + byte_col + (k / 2)]);
        }
        for (int i = lane_id; i < (WMMA_N * K_BYTES) / 16; i += THREADS_PER_WARP) {
            const int n = (i * 16) / K_BYTES;
            const int byte_col = (i * 16) % K_BYTES;
            cp_async16(&Bs[prefetch_buf][warp_id][n][byte_col],
                       &BT_b[(tile_col + n) * (K / 2) + byte_col + (k / 2)]);
        }
        asm volatile("cp.async.commit_group;");
    ...
    // Epilogue: compute penultimate, drain, then compute last.
    mma_int4_k64(rc0, ra, rb0);
    mma_int4_k64(rc1, ra, rb1);
...
void launch_kernel(const int8_t* A, const int8_t* BT, int32_t* C,
    ...
    int4_ptx_3stage_db<<<blocks, threads, 0, stream>>>(A, BT, C, cfg.M, cfg.N, cfg.K);
