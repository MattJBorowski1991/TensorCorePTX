...
// - Uses native ldmatrix.x2.trans for each B n8-half (8x64 int4 tile packed as 8x16 b16)
...
__device__ __forceinline__
void ldmatrix_b_k64(uint32_t rb[2], const uint16_t smem[][WMMA_N + PAD], int lane, int n_base) {
    const int r = lane % 16;
    const int c = n_base;
    const uint32_t addr = __cvta_generic_to_shared(&smem[r][c]);
    asm volatile(
...
__global__ void int4_ptx_mma_k64_db(
    ...
    // B stored as K_b16 x N b16 matrix in SMEM for ldmatrix.x2.trans.
    __shared__ __align__(16) uint16_t Bs[2][WARPS_PER_BLOCK][K_B16][WMMA_N + PAD];
    ...
    for (int i = lane_id; i < WMMA_N * K_B16; i += THREADS_PER_WARP) {
        const int kb16 = i / WMMA_N;
        const int n    = i % WMMA_N;
        const int src_idx = (tile_col + n) * (K / 2) + kb16 * 2;
        const uint16_t v = (uint16_t)(uint8_t)BT_b[src_idx]
                         | ((uint16_t)(uint8_t)BT_b[src_idx + 1] << 8);
        Bs[buf][warp_id][kb16][n] = v;
    }
    __syncthreads();
    ...
    for (int k = WMMA_K; k < K; k += WMMA_K) {
        ...
        for (int i = lane_id; i < WMMA_N * K_B16; i += THREADS_PER_WARP) {
            const int kb16 = i / WMMA_N;
            const int n    = i % WMMA_N;
            const int src_idx = (tile_col + n) * (K / 2) + (k / 2) + kb16 * 2;
            const uint16_t v = (uint16_t)(uint8_t)BT_b[src_idx]
                             | ((uint16_t)(uint8_t)BT_b[src_idx + 1] << 8);
            Bs[next][warp_id][kb16][n] = v;
        }
