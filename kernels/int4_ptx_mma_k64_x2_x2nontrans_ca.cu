...
// - Uses native ldmatrix.x2 x2 for A (16x64 int4 tile packed as 16x16 b16)
...
void ldmatrix_a_k64(uint32_t ra[4], const int8_t smem[][K_BYTES + PAD], int lane) {
    const int r = lane % 8;
    const uint32_t addr_top = __cvta_generic_to_shared(&smem[r][0]);
    const uint32_t addr_bot = __cvta_generic_to_shared(&smem[r + 8][0]);
    asm volatile(
