...
// - Uses native ldmatrix.x1 x4 for A (16x64 int4 tile packed as 16x16 b16)
...
void ldmatrix_a_k64(uint32_t ra[4], const int8_t smem[][K_BYTES + PAD], int lane) {
    const int r = lane % 8;
    const uint32_t addr_tl = __cvta_generic_to_shared(&smem[r][0]);
    const uint32_t addr_bl = __cvta_generic_to_shared(&smem[r + 8][0]);
    const uint32_t addr_tr = __cvta_generic_to_shared(&smem[r][16]);
    const uint32_t addr_br = __cvta_generic_to_shared(&smem[r + 8][16]);
    asm volatile(
