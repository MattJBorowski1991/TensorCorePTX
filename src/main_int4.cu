...
static void run_verify() {
    ...
    std::vector<int8_t>  h_A(Mv * (Kv / 2)), h_BT(Nv * (Kv / 2));
    std::vector<int32_t> h_C_ref(Mv * Nv, 0), h_C_out(Mv * Nv, 0);

    generate_int4_packed(h_A.data(), h_BT.data(), Mv, Nv, Kv, 1);
    cpu_gemm_int4(h_A.data(), h_BT.data(), h_C_ref.data(), Mv, Nv, Kv);

    DeviceBuffer<int8_t>  d_A(Mv * (Kv / 2));
    DeviceBuffer<int8_t>  d_BT(Nv * (Kv / 2));
    DeviceBuffer<int32_t> d_C(Mv * Nv);

    CHECK_CUDA(cudaMemcpy(d_A.get(),  h_A.data(),  Mv * (Kv / 2) * sizeof(int8_t), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_BT.get(), h_BT.data(), Nv * (Kv / 2) * sizeof(int8_t), cudaMemcpyHostToDevice));
    ...
    AccuracyResultI32 acc = measure_accuracy_int4(h_C_ref.data(), h_C_out.data(), Mv, Nv);
...
// ── Precision-loss measurement (FP32 → INT4 → GEMM → dequant → compare) ─────
static void run_precision_loss() {
    ...
    std::vector<int8_t>  h_A_q(M * (K / 2)), h_BT_q(N * (K / 2));
    ...
    float scale_A  = quantize_absmax_int4(h_A_fp32.data(),  h_A_q.data(),  M, K);
    float scale_BT = quantize_absmax_int4(h_BT_fp32.data(), h_BT_q.data(), N, K);

    DeviceBuffer<int8_t>  d_A(M * (K / 2));
    DeviceBuffer<int8_t>  d_BT(N * (K / 2));
    DeviceBuffer<int32_t> d_C(M * N);

    CHECK_CUDA(cudaMemcpy(d_A.get(),  h_A_q.data(),  M * (K / 2) * sizeof(int8_t), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_BT.get(), h_BT_q.data(), N * (K / 2) * sizeof(int8_t), cudaMemcpyHostToDevice));
...
int main() {
    ...
    if (skip_profile && skip_profile[0] != '\0' && skip_profile[0] != '0') return 0;
    ...
    GemmConfig cfg{.M=ProfilingSize, .N=ProfilingSize, .K=ProfilingSize,
                   .num_batches=4, .warmups=0, .runs=1};

    size_t szA  = (size_t)cfg.num_batches * cfg.M * (cfg.K / 2);
    size_t szBT = (size_t)cfg.num_batches * cfg.N * (cfg.K / 2);
    size_t szC  = (size_t)cfg.num_batches * cfg.M * cfg.N;

    std::vector<int8_t> h_A(szA), h_BT(szBT);
    generate_int4_packed(h_A.data(), h_BT.data(), cfg.M, cfg.N, cfg.K, cfg.num_batches);
    ...
    // INT4: same MACs formula; 2 MACs per packed byte pair processed
